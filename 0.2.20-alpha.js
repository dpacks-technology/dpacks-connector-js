            ...
            if (id !== 'dpacks-nightly-settings-nav') {
                ...
                axios.get(API_URL + `/api/v1/data-packets/check/${dpacks_key}/${pageId}/${id}`).then(function (response) {
                    if (response.data.exists === 0) {
                        ...
                        // xhr.open('GET', `${API_URL}/api/v1/data-packets/fetch/${dpacks_key}/${pageId}/${id}`);
                        // xhr.send();
                        ...
                        //
                        //             //axios.get(base_url + '/b2/' + dpacks_key + '/' + pageId + '/' + id)
...
messageWidget.innerHTML = 'Chat';
messageWidget.style.cursor = 'pointer';
...
messageWidget.style.color = 'white';
messageWidget.style.padding = '10px';
messageWidget.style.borderRadius = '50%';
messageWidget.style.width = '60px';
messageWidget.style.height = '60px';
messageWidget.style.display = 'flex';
messageWidget.style.justifyContent = 'center';
messageWidget.style.alignItems = 'center';
messageWidget.style.fontSize = '14px';
messageWidget.style.boxShadow = '0px 0px 10px rgba(0,0,0,0.25)';

// Append the widget to the body
document.body.appendChild(messageWidget);

// Variable to track chat visibility
let isChatOpen = false;

// Function to toggle chat visibility
function toggleChat() {
    const chatbot = document.querySelector('.chatbot'); // Select existing chatbot element
    if (isChatOpen) {
        chatbot.style.display = 'none'; // Hide chatbot if open
    } else {
        chatbot.style.display = 'block'; // Show chatbot if hidden
    }
    isChatOpen = !isChatOpen; // Toggle chat visibility state
}

// Add click event listener to the widget
messageWidget.addEventListener('click', toggleChat);

function webChat() {
    // Chatbot
    const chatbot = document.createElement('div');
    chatbot.classList.add('chatbot'); // Add a class for easier selection later
    chatbot.innerHTML = `<iframe src="https://dpacks.net/chat/${dpacks_key}" style="position: fixed; bottom: 0; right: 0; width: 300px; height: 400px; border: none; z-index: 999999;"></iframe>`;
